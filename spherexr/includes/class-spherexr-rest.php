...
class SphereXR_REST {
	...
	public function get_animation( $request ) {
		...
		if ( ! $post || $post->post_type !== 'spherexr_animation' ) {
			return new WP_Error( 'not_found', __( 'Animation not found.', 'spherexr' ), array( 'status' => 404 ) );
		}
	...
	public function create_animation( $request ) {
		...
		if ( ! $title ) {
			return new WP_Error( 'missing_title', __( 'Title is required.', 'spherexr' ), array( 'status' => 400 ) );
		}
		...
		update_post_meta( $post_id, '_spherexr_config', wp_json_encode( $default_config ) );
	...
	public function update_animation( $request ) {
		...
		if ( isset( $params['config'] ) ) {
			$clean = SphereXR_CPT::sanitize_config( $params['config'] );
			if ( ! $clean ) {
				return new WP_Error( 'invalid_config', __( 'Invalid configuration.', 'spherexr' ), array( 'status' => 400 ) );
			}
			update_post_meta( $post->ID, '_spherexr_config', wp_json_encode( $clean ) );
		}
